...
\begin{itemize}
    ...
    \item $overlap\_threshold$ (input parameter): Percentage value used to calculate penalties from edge overlap ratio
\end{itemize}

\textbf{Implementation Details:}
\begin{itemize}
    \item Input includes \texttt{overlap\_threshold} as percentage (e.g., 30\% means 0.30 ratio)
    \item Overlap penalty computed as: $penalty = overlap\_ratio \times (1 + overlap\_threshold/100)$
    \item Results are \textbf{sorted by path length} before returning to client
    \item Ensures consistent ordering: shortest to longest among diverse paths
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Sort final K paths by ascending length before output}
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Sequential Processing}: Queries processed \textbf{one by one}, not in parallel
    \item \textbf{Time Budget Check}: Before each query, check if time budget exceeded
    \item \textbf{Rejection Policy}: If budget exceeded, reject remaining queries immediately
    \item \textbf{Output Field}: Results use \texttt{approx\_shortest\_distance} (not \texttt{distance})
    \item \textbf{Rationale}: One-by-one processing enables early termination and fair resource allocation
\end{itemize}
...
\textbf{Key Implementation Requirements:}
\begin{itemize}
    \item Uses \texttt{overlap\_threshold} parameter from query (percentage value)
    \item Computes overlap as edge-sharing ratio between paths
    \item Final candidates \textbf{must be sorted by path length} (ascending order)
    \item Ensures predictable output ordering for testing and validation
\end{itemize}
